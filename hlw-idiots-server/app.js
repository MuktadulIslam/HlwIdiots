...
async function run() {
    try {
        ...
        // media add
        app.post("/media", verifyJWT, upload.single("file"), async (req, res) => {
            const data = {
        ...
        // questions bank by subject
        app.get("/questions/bank/subject/:subject", verifyJWT, async (req, res) => {
            let { page, limit } = req.query;
            page = parseInt(page);
            limit = parseInt(limit);
            const skip = page * limit;
        ...
        // monthly topper
        app.get("/monthly-toppers", verifyJWT, async (req, res) => {
            try {
