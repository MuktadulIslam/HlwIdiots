...
const QuestionSelectionCard = ({
    ...
    const isAdded = selectedQuestions.find((sq) =>
        sq === q._id ? true : false
    );
    ...
    return (
        <div className="relative border pr-10 py-5 pl-4 mb-5 rounded-xl text-sm">
            ...
            <div className="absolute top-2 right-2 flex flex-col justify-center items-center">
                <button
                    type="button"
                    onClick={() => selectedQuestionsHandler(q._id)}
                    className={`${
                        isAdded
                            ? "text-red-800 bg-red-100 hover:bg-red-200"
                            : "text-lime-800 bg-lime-100 hover:bg-lime-200"
                    } focus:ring-4 focus:outline-none font-medium text-sm p-2.5 text-center inline-flex items-center`}
                >
                    {isAdded ? (
